...
for (const [index, clip] of clips.clips.entries()) {
  ...
  writeFileSync(pcm, await cosyVoice3ZeroShotPcm(clip.spoken, cv3));
...
writeFileSync(path.join(root, "audio_meta.json"), `${JSON.stringify({tts_provider: "cosyvoice-clone", voice_id: "cosyvoice3-zh-male-ref", voices, total_duration_s: 165, delivery_note: "CosyVoice 3 zero-shot on cornerstone (Fun-CosyVoice3-0.5B-2512). Captions use weighted phrase timing over the final retimed audio."}, null, 2)}\n`);
