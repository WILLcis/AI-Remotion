...
import { generateLongCatAvatar } from "../src/avatar/longcat";

describe("LongCat avatar provider", () => {
  it("uploads a portrait and scene audio then atomically writes an avatar clip", async () => {
    const tempDir = mkdtempSync(path.join(tmpdir(), "ai-remotion-longcat-"));
    ...
    try {
      ...
      expect(requestUrl).toBe("http://127.0.0.1:8006/generate");
