...
- P1/P2 在官方 API 缺失时的 **assisted（半自动）** 降级：生成发布清单 + 打开官方创作者页说明。**默认仍是 Pack。** 浏览器 RPA 另受 `FLAG_video_publish_rpa` + 当次 `--i-accept-rpa-risk` 约束。
...
| 非官方协议 / 默认 RPA 发片 | **禁止作为默认实现** | ToS 与封号风险；可选 RPA 见 §13.2 |

**设计结论（P1）— 用户已确认长期 Pack**

- **不得**宣称「已接通视频号 OpenAPI」。
- 采用 **Publish Pack** 为默认（可选 RPA 见 §13.2）：  
  - 校验 MP4  
...
- 默认走 **Publish Pack** + 创作者后台人工确认。可选 RPA 见 §13.2。禁止 cookie / 私有 API。
...
1. **P1/P2 Publish Pack** — **默认**：视频号 / 小红书走「清单 + 人工发」（含义见 §13.1）。可选 RPA 见 §13.2。官方投稿 API 若日后开放，另开工单升级。
...
「不强上 RPA」= **默认**不自动填表、不自动点发布。

### 13.2 可选浏览器 RPA（双闸）

用户已接受封号与条款 residual risk。开启条件（缺一不可）：

1. `FLAG_video_publish_rpa={"enabled":true}`（默认 false；kill 即时关）。
2. 当次人话 `批准RPA` / `接受RPA风险`，Agent 才加 `--i-accept-rpa-risk`。即梦出片/发布同意 **不含** 此项。

实现：用本机 Google Chrome 持久 profile（非无痕、非 Playwright Chromium）打开官方创作者 URL，登录态只存在本机 `state/publish/rpa-profile/`。人类化输入与延迟；白天 10:00–20:00、每天最多 30 条、视频间隔 90 分钟（同条跨平台 2–5 分钟）；点「发表/发布」后必须等到成功确认才关浏览器。禁止 cookie 破解与非官方签名 API。失败时保留 Pack + 截图，改为人工发。
