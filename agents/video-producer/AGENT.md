...
3. **Talking-head / 我的形象 / 口播 / 数字人:** Dreamina identity. **Whether or not this is a scheduled hotspot**, run the cover pipeline: `image2image` cover (face only) → `multimodal2video` with that cover as `@Image 1`, face as `@Image 2`, timbre as `@Audio 1`, spoken in `{dialogue}`, captions in the prompt. Use `npm run media:dreamina -- talking-head --spoken "..." --out videos/<id> --generation-service dreamina` or `video:hotspot --format digital-human`. **Never** `text2video` and never `multimodal2video` on the face photo alone. Do not start HeyGen. Do not ask the four-way `generation.service` menu. Only use HeyGen if the user named HeyGen.
...
9. Execute as that primary until the next pending gate, then return `needs_approval`. Never approve gates yourself. Never call paid/cloud `heygen` or `final_render` without the user's current explicit approval. **Dreamina exception:** selecting `generation.service=dreamina` is paid-generation and publish consent — do not wait for 批准 storyboard / 批准付费 / 批准发布. **Talking-head / 口播 / 我的形象:** `npm run media:dreamina -- talking-head --spoken "..." --out videos/<id> --generation-service dreamina` (or `video:hotspot --format digital-human`). That command builds the cover still, uses it as `@Image 1`, and applies caption rules. **Never** `text2video` and never `multimodal2video` with only the face photo. Switch Dreamina logins with `media:dreamina switch --account <alias>` (not raw `dreamina login`; `dreamina session` is not a user account). Then `video:publish --platform all --generation-service dreamina --cover`. Flag `FLAG_dreamina_media`.
